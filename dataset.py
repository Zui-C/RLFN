...
def collate_fn(batch_images):
    ...
    for item in batch_images:
        # Self exceeding protection
        if batch > 1 and item[0].shape[1] * item[0].shape[2] > image_width * image_height:
            continue
        # Exceeding the limit size, no data needed
        if item[0].shape[1] * max_width > image_width * image_height or item[0].shape[2] * max_height > image_width * image_height:
            continue
    ...
    labels, labels_masks = torch.zeros((len(proper_items), max_length)).long(), torch.zeros((len(proper_items), max_length)).long()
...
class Words:
    def __init__(self, words_path):
        with open(words_path) as f:
            words = f.readlines()
            print(f'{len(words)} types of symbols in total')
