...
def train(params, model, optimizer, epoch, train_loader, writer=None):
    ...
    with tqdm(train_loader, total=len(train_loader)//params['train_parts']) as pbar:
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            ...
            probs, rec_loss, fusion_loss = model(images, image_masks, labels, label_masks,is_train=True)
            loss = rec_loss + fusion_loss
            loss.backward()
            ...
            if writer:
                ...
                writer.add_scalar('train/rec_loss', rec_loss.item(), current_step)
                writer.add_scalar('train/fusion_loss', fusion_loss.item(), current_step)
            ...
            pbar.set_description(f'Epoch: {epoch+1} train loss: {loss.item():.4f} rec loss: {rec_loss:.4f} '
                                 f'fusion loss: {fusion_loss:.4f} WordRate: {word_right / length:.4f} '
                                 f'ExpRate: {exp_right / cal_num:.4f}')
    ...
    with tqdm(eval_loader, total=len(eval_loader)//params['valid_parts']) as pbar, torch.no_grad():
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            images, image_masks, labels, label_masks = images.to(device), image_masks.to(
                device), labels.to(device), label_masks.to(device)
            batch, time = labels.shape[:2]
            probs, rec_loss, fusion_loss = model(images, image_masks, labels, label_masks, is_train=False)
            ...
            if writer:
                ...
                writer.add_scalar('eval/rec_loss', rec_loss.item(), current_step)
                writer.add_scalar('eval/fusion_loss', fusion_loss.item(), current_step)
